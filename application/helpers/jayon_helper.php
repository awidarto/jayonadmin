...
function get_thumbnail($delivery_id){
	...
	if(file_exists($CI->config->item('picture_path').$delivery_id.'.jpg')){
		if(file_exists($CI->config->item('thumbnail_path').'th_'.$delivery_id.'.jpg')){
			...
			$thumbnail = sprintf('<img style="cursor:pointer;" class="thumb" alt="'.$delivery_id.'" src="%s" /><br /><span class="rotate" name="'.$delivery_id.'" style="cursor:pointer;"  >rotate CW</span>',$thumbnail);					
		}else{
