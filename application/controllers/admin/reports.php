...
class Reports extends Application
{
    ...
    public function manifests($type = null,$deliverytype = null,$zone = null,$merchant = null,$exmerchant = null,$status = null,$year = null,$date_pickup = null ,$scope = null, $par1 = null, $par2 = null, $par3 = null,$par4 = null){

        $type = (is_null($type))?'Global':$type;
        $id = (is_null($type))?'noid':$type;
        $mid = (is_null($merchant))?'noid':$merchant;
        $exmid = (is_null($merchant))?'noid':$exmerchant;
        $deliverytype = (is_null($deliverytype))?'noid':$deliverytype;
        $status = (is_null($status))?'all':$status;
        ...
        if($exmid != 'noid'){
            ...
            $exmid = explode(':',$exmid);
            $this->db->where_not_in($this->config->item('assigned_delivery_table').'.merchant_id',$exmid);
        }
        ...
        foreach($rows->result() as $r){
            ...
            if($pdf == 'print' || $pdf == 'pdf'){
                ...
                $this->table->add_row(
                    ...
                    array('data'=>$this->hide_trx($r->merchant_trans_id).$fcode.'<br/>'.$this->date_did($r->delivery_id).'<hr/>'.$r->pickuptime,'class'=>'currency cod'),
    ...
    public function genmanifest(){
        ...
        $exmerchant = $this->input->post('exmerchant');
        $status = $this->input->post('status');
        ...
        $result = $this->manifests($type,$deliverytype ,$zone,$merchant,$exmerchant,$status,$year, $date_pickup ,$scope, $par1, $par2, $par3,$par4);
    ...
    public function deliveries($type = null,$deliverytype = null,$status = null,$year = null, $scope = null, $par1 = null, $par2 = null, $par3 = null,$par4 = null){
        ...
        */
        ...
        $this->breadcrumb->add_crumb('Delivery Report','admin/reports/deliveries');

        $page['ajaxurl'] = 'admin/reports/ajaxreconciliation';
        $page['page_title'] = 'Merchant Reconciliations';
        $data['select_title'] = 'Merchant';

        $data['controller'] = 'admin/reports/deliveries/';
        ...
        $pdffilename = 'JSM-DELIVERY-'.$mname.'-'.$data['invdatenum'];
        ...
        if($pdf == 'pdf' || $pdf == 'xls'){
            ...
            if($pdf == 'pdf'){
                $html = $this->load->view('print/deliveryprint',$data,true);
                $pdf_name = $pdffilename;
                $pdfbuf = pdf_create($html, $pdf_name,'A4','landscape', false);
            ...
            $this->load->view('print/deliveryprint',$data); // Load the view
        }else{
            $this->ag_auth->view('deliveriesgenerator',$data); // Load the view
        }
    }

    public function gendeliveries(){
        $type = null;
        $deliverytype = null;
        $status = null;
        $year = null;
        ...
        $result = $this->deliveries($type ,$deliverytype,$status,$year, $scope, $par1, $par2, $par3,$par4);
